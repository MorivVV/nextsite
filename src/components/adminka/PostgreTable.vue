...
export default {
  ...
  methods: {
    ...
    test() {
      this.$projectDebug(this.obj);
    },
    addField() {
      this.fields.push({ ...this.field });
      this.$projectDebug(this.field);
    },
    deleteRow(key, val) {
      let t = this.table;
      if (
      ...
      this.$projectDebug("Удаление ", key, val);
    },
    saveRow(key, val) {
      this.$projectDebug("обновление ", key, val);
      let t = this.table;
    ...
    viewFields(t) {
      ...
      this.fetchData({
        ...
        stateName: "postgre/fieldList"
      });
      this.getDataTable(t);
      this.fetchData({
        url: "api/restGet",
        data: {
          fields: ["ccu.column_name"],
          from: [
            "pg_catalog.pg_constraint:p",
            "pg_catalog.pg_class:c",
            "information_schema.constraint_column_usage:ccu"
          ],
          join: ["p.conrelid=c.oid", "p.conname=ccu.constraint_name"],
          filter: {
            "p.contype": "p",
            "c.relname": t,
            "ccu.table_schema": this.space
          },
          sort: ["ccu.column_name"]
        },
        stateName: "postgre/primarykey"
      });
    },
    getData() {
      this.loading = false;
      ...
      this.fetchData({
        ...
        stateName: "postgre/pg_namespace"
      });
      this.fetchData({
        url: "api/restGet",
        data: {
          ...
          filter: { schemaname: this.space },
          sort: ["tablename"]
        },
        stateName: "postgre/tableList"
      }).then(() => {
        this.loading = true;
      });
    }
