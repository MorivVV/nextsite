...
export default {
  ...
  methods: {
    ...
    gravity(obj){
      ...
      if (obj.y >= this.obj.height-this.blockSize*2) {
        obj.move = 0
        return;
      }
      if (this.checkMap(obj,[1, 2],bax)){
        obj.move = 0
        console.log("Бах");
        if (obj.vector > 0){
          return;
      ...
      obj.move = 0
      console.log(this.obj.height-this.blockSize*2);
      console.log(obj.x, obj.y, obj.vector);
      obj.y += Math.floor(this.blockSize * obj.vector / 100);
      obj.vector += 4;
    },
    getMaps() {
